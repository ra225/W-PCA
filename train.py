...
def main():
    ...
    print(args.tokenizer._tokens_to_ids(args.tokenizer._tokenize("What's up with the tokenizer?")))
    if use_gpu:
        model = model.cuda()
    if args.local_rank == 0:
        if args.model_name != "roberta_base":
            ...
            logging.info('Model size: {:.2f}M'.format((calc_params(model)-calc_params(model.lm_head)) / 1e6))
    ...
    optimizer = create_optimizer(model, args.optim_type, args.lr, args.weight_decay, args.momentum, args.betas)
