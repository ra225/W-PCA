...
parser.add_argument('--type_each_block', default=1, type=int, help='number of the type')
...
def main():
    ...
    for e in eval(args.ffn_expr):
        if e != eval(args.ffn_expr)[0]:
            issingle = False
            break
    #logging.info("main issingle={}".format(issingle))
    args.config = select_config(args.model_name, args.lowercase, issingle)
    model = select_model(args.model_name, args.lowercase, args.task, issingle=issingle)
    ...
    if final_heads != []:
        ...
        for i in range(args.config.num_layers):
            hidden_size = args.config.hidden_size
            if expr[i] >= args.type_each_block:
                hidden_size = int(0.25 * hidden_size)
            model.module.encoder[i][expr[i]].attention.num_attn_heads = final_heads[i]
            model.module.encoder[i][expr[i]].attention.attn_head_size = hidden_size // final_heads[i]
            model.module.encoder[i][expr[i]].attention.all_head_size = model.module.encoder[i][
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(tqdm(data_loader)):
            ...
            if args.task in glue_tasks + multi_choice_tasks:
                ...
                if args.model_name in nas_bert_models:
                    outputs, _, _ = model(token_ids, segment_ids, position_ids, attn_mask, args.type_blocks, eval(args.ffn_expr))
                else:
                    outputs, _, _ = model(token_ids, segment_ids, position_ids, attn_mask, args.type_blocks, eval(args.ffn_expr))
