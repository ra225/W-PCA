...
parser.add_argument('--final_heads', default='[]', type=str, help='number of heads when cutoff')
...
parser.add_argument('--total_epochs', default=20, type=int, help='total epochs')
...
parser.add_argument('--type_blocks', default=6, type=int, help='number of the type')
parser.add_argument('--type_each_block', default=3, type=int, help='number of the type')
...
def main():
    ...
    issingle = True
    for e in eval(args.ffn_expr):
        if e != eval(args.ffn_expr)[0]:
            issingle = False
            break
    teacher_config = select_config(args.teacher_model, args.lowercase, issingle)
    student_config = select_config(args.student_model, args.lowercase, issingle)
    teacher_model = select_model(args.teacher_model, args.lowercase, args.task, return_hid=True, issingle=issingle)
    student_model = select_model(args.student_model, args.lowercase, args.task, return_hid=True, issingle=issingle)
    ...
    #logging.info("args.total_epochs={}".format(args.total_epochs))
    ...
    if ckpt_path:
        if os.path.exists(ckpt_path):
            if args.official_pretrain:
                ...
                load_pretrain_state_dict(args.student_model, student_model, ckpt_path, use_gpu, is_finetune=True, task=args.task)
            if args.local_rank == 0:
    ...
    final_heads = eval(args.final_heads)
    if final_heads != []:
        ...
        for i in range(args.num_student_layers):
            ...
            student_model.module.encoder[i][expr[i]].attention.all_head_size = student_model.module.encoder[i][
                                                                                   expr[i]].attention.attn_head_size * \
                                                                               final_heads[i]
    try:
...
def train_epoch(teacher_model, student_model, epoch, optimizer, scheduler, train_loader, dev_loader, best_results, use_gpu):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        student_outputs = student_model(token_ids, segment_ids, position_ids, attn_mask, args.type_blocks, eval(args.ffn_expr))

        loss, attn_loss, ffn_loss, pred_loss, avg_sim = calc_distil_losses(teacher_outputs, student_outputs, use_gpu, labels, attn_mask, is_calc_pca_cka=batch_idx % args.calc_freq == 0)

        if batch_idx % args.calc_freq == 0:
            sim_another_batch = []
            for _ in range(args.type_blocks):
                sim_another_batch.append([0] * len(eval(args.ffn_expr)))
            layer_idx = 0
            for arch_id in eval(args.ffn_expr):
    ...
    with torch.no_grad():
        if args.task in glue_tasks:
            for batch_idx, data in enumerate(data_loader):
                ...
                outputs = model(token_ids, segment_ids, position_ids, attn_mask, args.type_blocks, eval(args.ffn_expr))
...
def calc_distil_losses(teacher_outputs, student_outputs, use_gpu, labels=None, attn_mask=None, is_calc_pca_cka = False):
    if args.task in glue_tasks:
        ...
        student_start_logits, student_end_logits, student_attn_outputs, student_ffn_outputs, _, _  = student_outputs

        attn_mask = deepcopy(attn_mask)
        attn_mask[:, 0] = 1
        teacher_start_logits = teacher_start_logits + attn_mask * -10000.0
        teacher_end_logits = teacher_end_logits + attn_mask * -10000.0
        student_start_logits = student_start_logits + attn_mask * -10000.0
        student_end_logits = student_end_logits + attn_mask * -10000.0
