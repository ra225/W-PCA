...
class MultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        #import logging
        #logging.info('embed_dim: {}, self.kdim:{}, self.vdim:{}'.format(embed_dim, self.kdim, self.vdim))
        #all 264
    ...
    def forward(
        ...
        if (
            ...
            #logging.info("if is true")
            assert key is not None and value is not None
            return_value = F.multi_head_attention_forward(
                query,
                key,
                value,
                self.embed_dim,
                ...
                is_from_mysupernet = True,
            )
            #for i in range(len(return_value)):
            #    logging.info("i={}, return_value[i].shape={}",i, return_value[i].shape)
            return return_value
        ...
        if before_softmax:
            logging.info('attn_weights.shape={}, v.shape={}'.format(attn_weights.shape, v.shape))
        ...
        import logging
        logging.info('attn.shape={}, attn_weights.shape={}'.format(attn.shape, attn_weights.shape))
